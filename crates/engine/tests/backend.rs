...
use envboard_engine::{EngineSpec, InstanceState, LineWriter, Listen, ProxyEngine, sha256};
...
async fn settle(engine: &EngineBackend, handle: &envboard_engine::EngineHandle) -> InstanceState {
    for _ in 0..200 {
...
async fn backend_lifecycle_through_the_trait_face() {
    ...
    assert_eq!(error.code, envboard_engine::ErrorCode::NotFound);
...
async fn double_register_conflicts_without_touching_the_first() {
    ...
    assert_eq!(error.code, envboard_engine::ErrorCode::Conflict);
    assert_eq!(settle(&engine, &handle).await, InstanceState::Running);
    engine.stop(&handle).await.unwrap();
    std::fs::remove_dir_all(&dir).ok();
}
