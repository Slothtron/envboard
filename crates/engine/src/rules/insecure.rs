//! `insecure_hosts` 的归一化与匹配 —— 契约见 `spec/capabilities.md` 的
//! 「insecure_hosts（按域名放宽上游校验）」一节。
//!
//! 与 [`super::host`] 的规则归一化**刻意不同**的两点，写在这里免得被"顺手统一"掉：
//!
...
///
/// 刻意不调用 [`super::host::normalize_host`]：那个函数会剥掉前导 `*.`。
...
pub fn validate_insecure_host(value: &str) -> Option<String> {
    ...
    super::host::validate_normalized_host(&host).then_some(host)
}
