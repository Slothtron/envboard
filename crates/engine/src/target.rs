...
//!   没有任何"全局关校验"的表达路径。
//! * chained_proxy：上游二级代理。由配置播种（环境 `upstream` 绑定），
//!   引擎按到达态执行 —— 经代理 CONNECT / absolute-URI 建连。
...
// SNI 改写（Mirror 的 SNI/Host/证书 CN 三策略）是后续扩展位；二级代理已由
// 配置播种。种子与判定语义已把接口钉死，扩展只加不改。
