...
use envboard_engine_fake::FakeEngine;
...
async fn update_refuses_identity_changes_while_running_and_applies_them_when_stopped() {
    ...
    assert_eq!(
        ...
        Some(&envboard_engine::domain::Desired::Stopped)
    );
...
async fn reconcile_survives_a_held_port_as_conflict_not_reassignment() {
    ...
    prepare_state(&harness, |state| {
        state
            .desired
            .insert("beta".into(), envboard_engine::domain::Desired::Running);
    });
...
fn create_probes_only_until_the_first_free_port() {
    ...
    let clock: Arc<dyn ClockPort> = Arc::new(envboard_engine::ManualClock::new(1_000));
    let manager = Manager::new(
        config,
...
fn a_dangling_upstream_reference_in_the_state_file_refuses_to_load() {
    ...
    let engine: Arc<dyn envboard_engine::ProxyEngine> = Arc::new(FakeEngine::new());
    ...
    let loaded = Manager::new(
        ...
        Arc::new(SocketPortProbe),
        Arc::new(RealFiles),
        harness.clock.clone() as Arc<dyn envboard_engine::ClockPort>,
        harness.logger.clone() as Arc<dyn envboard_engine::LoggerPort>,
