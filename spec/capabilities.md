...
探活、CA 物化、浏览器行为）不进契约，由 `README.md` 的支持矩阵与 `scripts/verify.py`
的实机层覆盖 —— 这条边界与 v1 一致。
...
| 清单 | 宿主 | 结果 | 钉住的语义 |
|---|---|---|---|
| `["app.example.com"]` | `app.example.com` | ✅ | 精确命中 |
| `["app.example.com"]` | `App.Example.COM.` | ✅ | 两侧都归一化 |
| `["example.com"]` | `app.example.com` | ❌ | 不做子域继承 |
| `["example.com"]` | `example.com.evil` | ❌ | 不做后缀匹配 |
| `[]` | 任意 | ❌ | 空 = 不放行 |
| `["10.0.0.8"]` | 无 SNI，上连地址 = 该 IP | ✅ | 无 SNI 时才退回地址 |
| `["10.0.0.8"]` | SNI = `app.example.com`，地址 = 该 IP | ❌ | SNI 存在就不退回地址 |
...
控制面审计事件（`events.jsonl`，权威仍是 state.json）与数据面请求轨迹
（`trajectories/<env>.jsonl`）的信封、词汇表、fail-closed 读契约与发射点纪律
见 `spec/events.md`（唯一出处，这里不重复）。
