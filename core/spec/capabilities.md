...
在 core 里定义**一次**。宿主配置只做映射，适配器**禁止**再写一份默认值。
...
改了 `listen`、`rules` 绑定或 `options` 即作废**旧的失败标记**（`port_conflict` /
`config_mismatch`）：那两个标记陈述的是"上一个配置失败了"，留着它会让界面拿**新**配置
报旧冲突（`config_mismatch` 尤其可能正是由某个选项引起的）。
