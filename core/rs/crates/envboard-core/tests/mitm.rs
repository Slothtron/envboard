...
fn mitmproxy_format_rsa_ca_loads_and_pairs() {
    ...
    let cn = envboard_core::der::cert_subject_cn(ca.ca_cert_der()).unwrap();
...
async fn corrupt_ca_is_rematerialised_with_a_visible_reason() {
    ...
    let reason = match outcome {
        envboard_core::CaOutcome::Regenerated { reason, .. } => reason,
...
async fn negotiate(ca: &Arc<SharedCa>, sni: &str) -> Vec<Vec<u8>> {
    let server_config = envboard_core::mitm_server_config(ca.clone()).unwrap();
