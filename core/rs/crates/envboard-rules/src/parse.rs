//! hosts 风格输入的解析 —— 契约见 `core/spec/rules.md`。
//!
//! **与 Python 实现逐字节对齐**是这个模块的核心要求（不是"大致一样"）：
//! 同一批 `core/spec/fixtures/rules/*.json`，Rust 与 Python 两份实现都要跑，
//! 渲染输出必须逐字节一致。所以这里刻意照着 `rules.md` 的 BNF 与
//! 跳过原因码写，而不是"写个更漂亮的解析器"。

use std::collections::BTreeMap;

use envboard_core_api::{is_ip_literal, parse_ip_literal, strip_brackets};
...
impl RulesImport {
    ...
    /// 契约里的结果载荷（`core/spec/rules.md` §7），用于契约测试与 REST 面。
...
pub fn parse_hosts_text(text: &str) -> RulesImport {
    ...
    for (index, raw) in split_lines(text).into_iter().enumerate() {
        ...
        // 保留作者写的拼法（`core/spec/rules.md` §3.2「拼法保留」）：只剥方括号，不做 IP 规范化。
