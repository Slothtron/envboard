...
//!
//! 契约见 `core/spec/capabilities.md` 的「上游代理（chained proxy）」一节。
...
impl UpstreamProxy {
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, Error> {
        ...
        crate::environment::validate_upstream_name(&name)
        ...
        let host = envboard_rules::validate_insecure_host(&raw_host).ok_or_else(|| {
            Error::invalid_config(
                &host_field,
