...
> 在每次实机层运行时重验。
>
> 表单后来新增过"每实例选项"这一组字段（一个显式开关 + 一个「其他实例选项」文本框），
> 运行中与名字/端口/绑定一起被锁住 —— 下方第 1 节是**当时**四字段表单的观测记录，
> 字段数一处按当时情形保留。
>
> **再后来的变化（本文档不再跟改）**：那一组"任意选项"字段已经**删除**（它是一条绕过
> 契约的任意通道），改为按域名放宽上游证书校验的 `insecure_hosts`；`rules` **绑定**也从
> "必须停机"变成**热**字段。当前的字段清单、热/停机矩阵与错误路径以
> `core/spec/capabilities.md` 的「环境（Environment）」与「编辑已有环境（PATCH 语义）」
> 两节为准；实机断言由 `scripts/verify_live_v2.py` 的第 10、12、14 组在实机层重验。
