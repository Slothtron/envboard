...
use envboard_engine::rules::{parse_hosts_text, render_rules};
use envboard_engine::{ErrorCode, InstanceState};
...
/// 能力 → fixture 目录。这份注册表就是契约的一部分，改它要同时改 `spec/`。
...
fn assert_error(case: &Case, error: envboard_engine::Error) {
    let expect = case.expect.as_object().expect("expect must be an object");
    assert_ne!(
        expect.get("ok"),
...
fn proxy_validate_matches_the_contract() {
    for case in load_cases("proxy") {
        match envboard_engine::domain::UpstreamProxy::from_json(&case.input) {
            Ok(proxy) => {
                assert_ne!(
                    case.expect["ok"],
                    Value::Bool(false),
...
fn port_allocate_matches_the_contract() {
    for case in load_cases("ports") {
        ...
        let request = PortRequest {
            ...
            range: range.unwrap_or(envboard_engine::domain::DEFAULT_PORT_RANGE),
            max_attempts: input
                .get("max_attempts")
                .and_then(Value::as_u64)
                .map(|raw| raw as usize)
                .unwrap_or(envboard_engine::domain::DEFAULT_MAX_ATTEMPTS),
        };
...
fn insecure_hosts_matches_the_contract() {
    for case in load_cases("insecure") {
        ...
        let actual =
            json!({ "match": envboard_engine::rules::insecure_matches(&hosts, sni, address) });
        assert_fields(&case, &actual);
    }
}
...
fn fixture_directories_match_the_capability_registry() {
    ...
    assert_eq!(
        total, 79,
        "契约 fixture 总数变了：请同时更新 spec/ 与 README 里的数字"
    );
}
...
fn the_orchestration_surface_is_core_neutral() {
    ...
    let capabilities = envboard_engine::CoreCapabilities {
        listen: true,
        http1_only: true,
        ..Default::default()
    };
