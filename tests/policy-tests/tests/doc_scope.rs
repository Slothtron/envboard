...
//!
//! 本仓的入库文档是 `README.md` / `CHANGELOG.md` / `spec/**` / `docs/**` 与各 crate；
...
//!    本仓内的文件（`spec/…`、`capabilities.md`、`README.md` …）或写"本文件" ——
...
/// （它们照旧扫这些文件）兜住。
const SELF_SOURCES: &[&str] = &["tests/policy-tests/tests/doc_scope.rs"]; // doc-scope-lint: allow
...
fn path_references(line: &str) -> Vec<String> {
    ...
    for prefix in PATH_PREFIXES {
        ...
        while let Some(offset) = line[from..].find(prefix) {
            ...
            // 前缀必须落在词边界：`policy-tests/` 里嵌着的 `tests/` 不是路径起点，
            // 否则合法引用 `tests/policy-tests/tests/x.rs` 会被再切出两条假引用。
