...
- `core/spec/capabilities.md` **重写**：领域模型从 v1 的 8 字段收为 5 字段
  （`name` / `listen` / `rules` / `options` / `description`）。删除 `dns_servers`、`hosts`、
...
- 契约 fixture **重新基线化**：v1 的 11 个 `environment/` + 2 个 `merge/` 全部重写
  ...
  现状：`environment/` 23 + `merge/` 8 + `rules/` 8（**原样保留**）+ `ports/` 6
  + `lifecycle/` 6 = **51** 个。
...
- **Rust 核心（v2 M0 + M1）**：多环境管理器的实现落地，由 7 个 crate 组成，
  ...
  - `envboard-domain`：5 字段环境模型（校验/归一化/PATCH 合并）、端口选择
  ...
  - `envboard-contract-tests`：**消费 `core/spec/fixtures` 全部 51 个 golden case**。
